...
import { CrmLeadsClient } from "./_components/CrmLeadsClient";
...
async function getData() {
  ...
  if (leadsErr) {
    throw new Error(`Veritabanı hatası: ${leadsErr.message}`);
  }
...
export default async function CrmLeadsPage({
  ...
  searchParams: Promise<{ convert_audit?: string; convert_contact?: string }>;
}) {
  try {
    ...
    return (
      <div style={{ padding: "32px 0" }}>
        <h1 style={{ margin: "0 0 16px", fontSize: 17, fontWeight: 600, color: "var(--c-text)" }}>Müşteri Adayları</h1>
        <div style={{
          ...
          <div style={{ marginTop: 16, fontSize: 12, color: "var(--c-dim)" }}>
            Çözüm: Supabase Dashboard → SQL Editor → <code style={{ background: "var(--c-border)", padding: "2px 6px", borderRadius: 4 }}>supabase/crm-lead-management.sql</code> şemasını çalıştırın.
          </div>
        </div>
      </div>
    );
