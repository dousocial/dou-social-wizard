  ...
  return (
    <footer className="border-t border-mute-100 bg-paper">
      <Container size="wide" className="py-16">
        ...
        <div className="mt-12 flex flex-wrap gap-x-5 gap-y-2 border-t border-mute-100 pt-8">
          {RELATED.map((r) => (
            <Link
              key={r.href}
              href={r.href as never}
              className="text-xs text-mute-400 transition hover:text-ink"
            >
              {r.label}
            </Link>
          ))}
        ...
        <div className="mt-6 flex flex-col items-start justify-between gap-4 border-t border-mute-100 pt-6 text-xs text-mute-500 md:flex-row md:items-center">
